...
// TAKEN FROM http://www.codeproject.com/system/listports.asp 
 ...
 */
{
  ...
  for(dwIndex=0;;++dwIndex) {
    ...
    if (_tcsnicmp(lpPortName,TEXT("COM"),3)!=0) continue; // We want only COM serial ports
  ...
  for(dwIndex1=0;;++dwIndex1){
    ...
    for(dwIndex2=0;;++dwIndex2){
      ...
      for(dwIndex3=0;;++dwIndex3){
        ...
        if(_tcsnicmp(lpPortName,TEXT("COM"),3)!=0)continue;

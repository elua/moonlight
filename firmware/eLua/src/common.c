...
{
  u32 i;
  const u8 *pdata = ( const u8* )p;
  
  for( i = 0; i < count; i ++ )
    platform_uart_send( id, pdata[ i ] );
  return count;              
}
...
#ifdef BUF_ENABLE_UART
  if( buf_is_enabled( BUF_ID_UART, id ) )
    return buf_get_count( BUF_ID_UART, id ) > 0; 
  else
#endif
  return platform_s_uart_data_ready( id );
}
...
int platform_uart_set_buffer( unsigned id, unsigned log2size )
{
  ...
  if( log2size == 0 )
  {
    ...
    return id < SERVICE_ID_FIRST ? platform_s_uart_set_buffer( id, BUF_SIZE_NONE ) : PLATFORM_OK;
  }  
  else
  {
    // Enable buffering
    if( buf_set( BUF_ID_UART, id, log2size, BUF_DSIZE_U8 ) == PLATFORM_ERR )
      return PLATFORM_ERR;
    return id < SERVICE_ID_FIRST ? platform_s_uart_set_buffer( id, 1 << log2size ) : PLATFORM_OK;      
  }
#else
  return PLATFORM_ERR;
#endif

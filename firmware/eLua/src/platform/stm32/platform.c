...
// Pass-through data
#define PLATFORM_PASSTHROUGH_PORT       2
#define PLATFORM_PASSTHROUGH_PIN        13
#define PLATFORM_PASSTHROUGH_UART1_ID   0
#define PLATFORM_PASSTHROUGH_UART2_ID   1
static int platform_passthrough_mode;
...
{ 
  ...
  platform_pio_op( PLATFORM_PASSTHROUGH_PORT, 1 << PLATFORM_PASSTHROUGH_PIN, PLATFORM_IO_PIN_DIR_INPUT );
  if( platform_pio_op( PLATFORM_PASSTHROUGH_PORT, 1 << PLATFORM_PASSTHROUGH_PIN, PLATFORM_IO_PIN_GET ) == 1 )
  {
    platform_pio_op( PLATFORM_PASSTHROUGH_PORT, 1 << 11, PLATFORM_IO_PIN_DIR_OUTPUT );
    platform_pio_op( PLATFORM_PASSTHROUGH_PORT, 1 << 11, PLATFORM_IO_PIN_SET );      
    // Enter pass-through mode, can only be disabled with a reset 
    platform_passthrough_mode = 1;
    // Setup both serial ports
    platform_uart_setup( PLATFORM_PASSTHROUGH_UART1_ID, 115200, 8, PLATFORM_UART_PARITY_NONE, PLATFORM_UART_STOPBITS_1 );
    platform_uart_set_buffer( PLATFORM_PASSTHROUGH_UART1_ID, 10 );
    platform_uart_setup( PLATFORM_PASSTHROUGH_UART2_ID, 115200, 8, PLATFORM_UART_PARITY_NONE, PLATFORM_UART_STOPBITS_1 );
    platform_uart_set_buffer( PLATFORM_PASSTHROUGH_UART2_ID, 10 );        
    while( 1 )
    {
      if( buf_read( BUF_ID_UART, PLATFORM_PASSTHROUGH_UART1_ID, &data ) == PLATFORM_OK )
        platform_uart_send( PLATFORM_PASSTHROUGH_UART2_ID, data );
      if( buf_read( BUF_ID_UART, PLATFORM_PASSTHROUGH_UART2_ID, &data ) == PLATFORM_OK )
        platform_uart_send( PLATFORM_PASSTHROUGH_UART1_ID, data );      
    }
  }                                                                 
  else
    platform_passthrough_mode = 0;
  ...
  drv_disp_init();
  sircs_init();    
  // Pullup the UART CTS line (PA11)
  platform_pio_op( 0, 1 << 11, PLATFORM_IO_PIN_PULLUP );
...
static void RCC_Configuration(void)
{
  ...
  // Enable UART clocks here (to accomodate the passthrough mode more easily)
...
static void NVIC_Configuration(void)
{
  ...
  const u32 usart_irqs[] = { USART1_IRQn, USART2_IRQn, USART3_IRQn, UART4_IRQn, UART5_IRQn };
  unsigned i;
  ...
  for( i = 0; i < sizeof( usart_irqs ) / sizeof( u32 ); i ++ )
  {
    nvic_init_structure.NVIC_IRQChannel = usart_irqs[ i ];
...
static void all_usart_irqhandler( int usart_id )
{
  ...
  if( platform_passthrough_mode == 0 )
    cmn_rx_handler( usart_id, USART_ReceiveData( usart[ usart_id ] ) );
  else
  {
    data = USART_ReceiveData( usart[ usart_id ] );  
...
static u8 wiz_had_escape, wiz_real_char;

static void wiz_usart_irqhandler( int usart_id )
{
  u8 data;
  
  if( platform_passthrough_mode == 1 )
  {
    data = USART_ReceiveData( usart[ usart_id ] );  
    buf_write( BUF_ID_UART, usart_id, ( t_buf_data* )&data );
    return;
  ...
  {
    wiz_had_escape = 0;
    data = data ^ WIZ_ESC_MASK;
  }
  else if( data == WIZ_ESC_CHAR )
  {
    wiz_had_escape = 1;
    wiz_real_char = 0;
  }
  else if( data == WIZ_HAS_CGI_CHAR )
  { 
    wizc_set_cgi_flag( 1 );
    wiz_real_char = 0;
  }
...
#endif // #ifdef WIZ_UART_ID

void USART1_IRQHandler()
{
#if defined( WIZ_UART_ID ) && WIZ_UART_ID == 0
  wiz_usart_irqhandler( 0 );
#else
...
{
#if defined( WIZ_UART_ID ) && WIZ_UART_ID == 1
  wiz_usart_irqhandler( 1 );
#else
...
{
#if defined( WIZ_UART_ID ) && WIZ_UART_ID == 2
  wiz_usart_irqhandler( 2 );
#else
...
{
#if defined( WIZ_UART_ID ) && WIZ_UART_ID == 3
  wiz_usart_irqhandler( 3 );
#else
...
{
#if defined( WIZ_UART_ID ) && WIZ_UART_ID == 4
  wiz_usart_irqhandler( 4 );
#else
...
{
  USART_ITConfig( usart[ id ], USART_IT_RXNE, size == 0 ? DISABLE : ENABLE );
  return PLATFORM_OK;
}
...
#ifdef WIZ_UART_ID
  if( WIZ_UART_ID == id && platform_passthrough_mode == 0 )  
    GPIO_InitStructure.GPIO_Pin = usart_gpio_tx_pin[id] | ( 1 << 12 );
  else
#endif
...
static void uarts_init()
{
  ...
  USART_ITConfig( usart[ SERMUX_PHYS_ID ], USART_IT_RXNE, ENABLE );   
...
#if defined( WIZ_UART_ID )
  if( id == WIZ_UART_ID && platform_passthrough_mode == 0 )
...
{
  return USART_GetFlagStatus( usart[ id ], USART_FLAG_RXNE ) == SET;
}
...
// TIM8 is used for PWM generation, so it's not included here (see PWM section)
...
#define PWM_BASE_CLK            ( HCLK / PCLK2_DIV )
...
{
  return PWM_BASE_CLK / ( TIM_GetPrescaler( PWM_TIMER ) + 1 );
}
...
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
  TIM_TypeDef* ptimer = PWM_TIMER;
  u16 pre = PWM_BASE_CLK / clock;
...
u32 platform_pwm_setup( unsigned id, u32 frequency, unsigned duty )
{
  ...
  ptimer->ARR = platform_pwm_get_clock( id ) / frequency - 1;
  TIM_ARRPreloadConfig( ptimer, ENABLE );
  
  /* PWM Mode configuration */  
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
  TIM_OCInitStructure.TIM_OutputState = ( ptimer->CCER & ( ( u16 )1 << 4 * id ) ) ? TIM_OutputState_Enable : TIM_OutputState_Disable;
  TIM_OCInitStructure.TIM_OutputNState = TIM_OutputNState_Disable;
  TIM_OCInitStructure.TIM_Pulse = ( u16 )( duty * ( ptimer->ARR + 1 ) / 100 );
  ...
  return platform_pwm_get_clock() / ( ptimer->ARR + 1 );
}
...
extern void cortex_disable_interrupts();
extern void cortex_enable_interrupts();
...
#include "lrodefs.h"
extern const LUA_REG_TYPE dp105_map[];
extern const LUA_REG_TYPE sircs_map[];
extern const LUA_REG_TYPE snd_map[];
extern int luaopen_dp105( lua_State *L );

const LUA_REG_TYPE platform_map[] =
{
#if LUA_OPTIMIZE_MEMORY > 0
  { LSTRKEY( PS_DP105_NAME ), LROVAL( dp105_map ) },
  { LSTRKEY( PS_SIRCS_NAME ), LROVAL( sircs_map ) },
  { LSTRKEY( PS_SND_NAME ), LROVAL( snd_map ) },
#endif
  { LNILKEY, LNILVAL }
};
...
LUALIB_API int luaopen_platform( lua_State *L )
{
  ...
  lua_newtable( L );
  luaL_register( L, NULL, dp105_map );
  lua_setfield( L, -2, PS_DP105_NAME );
  lua_newtable( L );
  luaL_register( L, NULL, sircs_map );
  lua_setfield( L, -2, PS_SIRCS_NAME );  
